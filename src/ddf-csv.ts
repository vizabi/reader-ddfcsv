...
const applyFilterRow = (row, filter) => {
...
const throwError = (error: DdfCsvError, logger) => {
 ...
 */
const processWhere = (where, joinFilters) => {
  const result = {};
...
const querySchema = (queryParam, {datapackage}) => {
  ...
  if (datapackage.ddfSchema[collection]) {
    ...
    throwError(new DdfCsvError(DDF_ERROR, `No valid collection (${collection}) for schema query`), this.logger);
  }
};

const fillMissingValues = (row, projection) => {
 ...
 */
const filterConceptsByType = (conceptTypes, queryKey, options) => {
...
const setConceptsLookup = (concepts, options) => {
 ...
 */
const getEntityConceptRenameMap = (queryKey, resourceKey, options) => {
...
const projectRow = (row, projectionSet) => {
  const result = {};
...
const renameHeaderRow = (row, renameMap) => {
  const result = {};
...
const joinRow = (resultRow, sourceRow, mode) => {
  switch (mode) {
    case 'overwrite':
    ...
    case 'overwriteWithError':
      ...
      for (const concept in sourceRow) {
        if (resultRow[concept] !== undefined && resultRow[concept] !== sourceRow[concept]) {
          ...
          throwError(new DdfCsvError(DDF_ERROR, errStr), this.logger);
        } else {
          resultRow[concept] = sourceRow[concept];
        }
      }
...
const joinData = (key, joinMode, ...data) => {
  if (data.length === 1) {
    return data[0];
  }
...
const createKeyString = (key, row = false) => {
  const canonicalKey = key.slice(0).sort();
...
const getLanguages = ({datapackage}): string[] => {
...
const buildConceptsLookup = (concepts, options) => {
...
export class DdfCsvReader {
  private keyValueLookup = new Map<string, any>();
  private resourcesLookup = new Map();
  private optimalFilesSet = [];

  constructor(private logger?: any) {
  }

  async query(queryParam, baseOptions) {
    // console.log(JSON.stringify(queryParam, null, '\t'))
    let data;

    try {
      await validateQueryStructure(queryParam, baseOptions);
      baseOptions.datapackage = await this.loadDataPackage(baseOptions);
      await this.loadConcepts(queryParam, baseOptions);
  ...
  private loadDataPackage(baseOptions: IBaseReaderOptions): Promise<IDatapackage> {
    ...
    return new Promise((resolve, reject) => {
      baseOptions.fileReader.readText(datapackagePath, (err, data) => {
        ...
        try {
          ...
          this.optimalFilesSet = [];
          this.buildResourcesLookup(datapackage);
          this.buildKeyValueLookup(datapackage);
        } catch (parseErr) {
  ...
  private async loadConcepts(queryParam, options): Promise<object> {
    ...
    try {
      const concepts = await this.queryData(conceptQuery, options);
      ...
      result = await this.reparseConcepts(options);
    } catch (error) {
      throw error;
    }
    return result;
  ...
  private queryData(queryParam, options) {
    const {
      select: {key = [], value = []},
      from = '',
      where = {},
    ...
    const resourcesPromise = this.loadResources(select.key, [...select.value, ...filterFields], language, options);
    // list of entities selected from a join clause, later insterted in where clause
    const joinsPromise = this.getJoinFilters(join, queryParam, options);
    // filter which ensures result only includes queried entity sets
    const entitySetFilterPromise = this.getEntitySetFilter(select.key, queryParam, options);
    ...
    return Promise.all([resourcesPromise, entitySetFilterPromise, joinsPromise])
      .then(([resourceResponses, entitySetFilter, joinFilters]) => {
        ...
        // rename key-columns and remove irrelevant value-columns
          .map(response => this.processResourceResponse(response, select, filterFields, options));

        // join (reduce) data to one data table
        const queryResult = joinData(select.key, 'overwrite', ...dataTables)
        // apply filters (entity sets and where (including join))
          .filter(row => applyFilterRow(row, filter))
          // fill any missing values with null values
          .map(row => fillMissingValues(row, projection))
          // remove fields used only for filtering
  ...
  private getJoinFilters(join, queryParam, options) {
    return Promise.all(Object.keys(join).map(joinID =>
      this.getJoinFilter(joinID, join[joinID], queryParam, options)))
      .then(results => results.reduce(mergeObjects, {}));
  }

  private getJoinFilter(joinID, join, queryParam, options) {
   ...
   */
  private getEntitySetFilter(conceptStrings, queryParam, options) {
    const promises = filterConceptsByType(['entity_set'], conceptStrings, options)
      .map(concept => this.query({
   ...
   */
  private getResources(key, value?) {
    ...
    // one key, one value
    let oneKeyOneValueResourcesArray = this.keyValueLookup
      .get(createKeyString(key))
      .get(value);

    if (oneKeyOneValueResourcesArray) {
      oneKeyOneValueResourcesArray = oneKeyOneValueResourcesArray
        .filter(v => isEmpty(this.optimalFilesSet) || includes(this.optimalFilesSet, v.path));
    }

    return new Set(oneKeyOneValueResourcesArray);
  }

  private processResourceResponse(response, select, filterFields, options) {
    ...
    // remove fields not used for select or filter
      .map(row => projectRow(row, resourceProjection))
      // rename header rows (must happen **after** projection)
      .map(row => renameHeaderRow(row, renameMap));
  }

  private loadResources(key, value, language, options) {
    const resources = this.getResources(key, value);

    return Promise.all([...resources].map(
      resource => this.loadResource(resource, language, options)
    ));
  }

  private loadResource(resource, language, options) {
    const filePromises = [];

    if (typeof resource.data === 'undefined') {
      resource.data = this.loadFile(resource.path, options);
    }
    ...
    if (languageValid) {
      if (!languageLoaded) {
        ...
        resource.translations[language] =
          this.loadFile(translationPath, options).catch(err => Promise.resolve({}));
      }
  ...
  private loadFile(filePath, options) {
  ...
  private buildResourcesLookup(datapackagePar) {
    if (this.resourcesLookup.size > 0) {
    ...
    datapackagePar.resources.forEach(resource => {
      ...
      this.resourcesLookup.set(resource.name, resource);
    });

    return this.resourcesLookup;
  }

  private buildKeyValueLookup(datapackagePar) {
